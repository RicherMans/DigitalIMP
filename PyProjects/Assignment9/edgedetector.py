...
def main():
    ...
    misc.imsave('canny_prewitt', prewittfiltered)
    
    robertsfiltered = filter_canny(args.inputimage, 'roberts')
    misc.imsave('canny_roberts', robertsfiltered)
...
def filter_canny(inputimage, filttype):
    ...
    angle[angle.any()<22.5 and angle.any()>-22.5] = 0.
    angle[angle.any()>=22.5 and angle.any()<=67.5] = 45.
    angle[angle.any()>67.5 and angle.any()<112.5] = 90.
    angle[angle.any()<=-22.5 and angle.any()>-67.5] = -45.
#     maximum suppression
...
#                     weightedavg += transformedimg[i+(p-offset)][j+(q-offset)] * mask[p][q]
#             if weightedavg<0:
#                 weightedavg = 0
